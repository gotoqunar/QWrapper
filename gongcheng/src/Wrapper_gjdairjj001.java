...
public class Wrapper_gjdairjj001 implements QunarCrawler {
	private static final NameValuePair WDS_CORPORATE_SALES = new NameValuePair(
			"WDS_CORPORATE_SALES", "FALSE");
	private static final NameValuePair SO_SITE_ISSUE_TKT_PER_PAX = new NameValuePair(
			"SO_SITE_ISSUE_TKT_PER_PAX", "TRUE");
	private static final NameValuePair FORCE_OVERRIDE = new NameValuePair(
			"FORCE_OVERRIDE", "TRUE");
	private static final NameValuePair WDS_DOMAIN_NAME = new NameValuePair(
			"WDS_DOMAIN_NAME", "tam.com.br");
	private static final NameValuePair WDS_MARKET = new NameValuePair(
			"WDS_MARKET", "OC");
	private static final NameValuePair WDS_DISABLE_ATC_CHANGE_ITIN = new NameValuePair(
			"WDS_DISABLE_ATC_CHANGE_ITIN", "TRUE");
	...
	private static final NameValuePair WDS_ONLINE_OPINION_EXIT_PERCENT = new NameValuePair(
			"WDS_ONLINE_OPINION_EXIT_PERCENT", "10");
	private static final NameValuePair WDS_ACI_ENABLED_MARKETS = new NameValuePair(
			"WDS_ACI_ENABLED_MARKETS", "BR:OP:CO");
	private static final NameValuePair WDS_MARKET_WITH_INSURANCE = new NameValuePair(
			"WDS_MARKET_WITH_INSURANCE", "BR:OP:CO:PE");
	private static final NameValuePair WDS_DISABLE_ATC_REFUND = new NameValuePair(
			"WDS_DISABLE_ATC_REFUND", "TRUE");
	private static final NameValuePair SITE = new NameValuePair("SITE",
			"JJBKJJBK");
	private static final NameValuePair LANGUAGE = new NameValuePair("LANGUAGE",
			"GB");
	private static final NameValuePair FROM_PAGE = new NameValuePair(
			"FROM_PAGE", "HOMESEARCH");
	private static final NameValuePair TRIP_TYPE = new NameValuePair(
			"TRIP_TYPE", "O");
	private static final NameValuePair ADULTS = new NameValuePair("adults", "1");
	private static final NameValuePair CHILDREN = new NameValuePair("children",
			"0");
	private static final NameValuePair INFANTS = new NameValuePair("infants",
			"0");
	private static final NameValuePair CORPORATE_CODE_INPUT = new NameValuePair(
			"CORPORATE_CODE_INPUT", "");
	private static final NameValuePair SEARCH_COOKIE = new NameValuePair(
			"SEARCH_COOKIE", "");
	...
	public static void main(String[] args) {
		...
		searchParam.setDepDate("2014-10-30");
		searchParam.setTimeOut("60000");
		searchParam.setToken("");

		String html = new Wrapper_gjdairjj001().getHtml(searchParam);

		ProcessResultInfo result = new ProcessResultInfo();
		result = new Wrapper_gjdairjj001().process(html, searchParam);
	...
	public BookingResult getBookingInfo(FlightSearchParam arg0) {
		...
		String depcountry = InfoCenter
				.getCountry2CodeFromNameZh(InfoCenter.getCountryFromCity(
						InfoCenter.getCityFromAirportCode(arg0.getDep()), "zh"));
		String arrcountry = InfoCenter
				.getCountry2CodeFromNameZh(InfoCenter.getCountryFromCity(
						InfoCenter.getCityFromAirportCode(arg0.getArr()), "zh"));

		String COMMERCIAL_FARE_FAMILY = "JJINTECO";
		if (depcountry.equalsIgnoreCase("BR")
				&& arrcountry.equalsIgnoreCase("BR")) {
	...
	public String getHtml(FlightSearchParam arg0) {
		...
		try {
			// 1. 生成请求
			// get all query parameters from the url set by
			// wrapperSearchInterface
...
//					proxy);
			// 2.
			// 模拟是否需要管理Cookie，CookiePolicy.BROWSER_COMPATIBILITY是按照浏览器的方式处理cookie
			...
			String depcountry = InfoCenter.getCountry2CodeFromNameZh(InfoCenter
					.getCountryFromCity(
							InfoCenter.getCityFromAirportCode(arg0.getDep()),
							"zh"));
			String arrcountry = InfoCenter.getCountry2CodeFromNameZh(InfoCenter
					.getCountryFromCity(
							InfoCenter.getCityFromAirportCode(arg0.getArr()),
							"zh"));
			...
			NameValuePair[] names = {
					...
					CHILDREN,
					INFANTS,
					new NameValuePair("COMMERCIAL_FARE_FAMILY_1",
							COMMERCIAL_FARE_FAMILY), CORPORATE_CODE_INPUT,
					SEARCH_COOKIE, };
			// 设置请求体
			post.setRequestBody(names);
			// 设置Referer请求体
			post.setRequestHeader("Referer",
					"http://book.tam.com.br/TAM/dyn/air/homeSearch");
			// 设置编码格式
	...
	public ProcessResultInfo process(String arg0, FlightSearchParam arg1) {
		...
		try {
			...
			if (beginindex < 0) {
				...
				String check = StringUtils
						.substringBetween(
								html,
								"<tbody class=\"none\" data-wdk-toggle-transition=\"none\">",
								"</div> </td>   </tr>     </tbody> </table>");
				// 去除剩余座位信息及耗时信息
				check = check.replaceAll(
						"<small class=\"ico seat em\">\\d</small>", "");
				...
				for (int i = 0; i < numberOfZhi; i++) {
					...
					int m = i * 8;
					System.out.println("Zhifei: " + i + " info: " + zhifei[m]
							+ " " + zhifei[m + 1] + " " + zhifei[m + 2] + " "
							+ zhifei[m + 3] + " " + zhifei[m + 4] + " "
							+ zhifei[m + 5] + " " + zhifei[m + 6] + " "
							+ zhifei[m + 7]);
					if (zhifei[m + 5].equals("OUT")) {
						if (zhifei[m + 6].equals("OUT")) {
							if (zhifei[m + 7].equals("OUT")) {
								...
								flightDetail.setPrice(Math.round(Float
										.parseFloat(zhifei[m + 7])));
			...
			if (beginindex1 < 0) {
				...
				int length = new String("<tr class=\"flight flightId-")
						.length();
				int end = -1;
				while (start > 0) {
					end = check.indexOf("<tr class=\"flight flightId-", start
							+ length);
					if (end > 0 && end > start) {
				...
				for (String tmp : flightsHtml) {
					...
					String no = StringUtils.substringBetween(tmp,
							"data-flight-number=\"", "\"");
					// String planeType =
					// StringUtils.substringBetween(tmp,"data-aircraft=\"",
					// "\"");
					String dep = StringUtils.substringBetween(tmp,
							"data-departureairportcode=\"", "\"");
					String arr = StringUtils.substringBetween(tmp,
							"data-arrivalairportcode=\"", "\"");
					String depTimeStr = StringUtils.substringBetween(tmp,
							"data-departuredate=\"", "\"");
					String arrTimeStr = StringUtils.substringBetween(tmp,
							"data-arrivaldate=\"", "\"");

					System.out.println("deptime : " + depTimeStr);
					System.out.println("arrtime : " + arrTimeStr);
					// 获取价格
					String pricebufs[] = getValues(tmp,
							"<td class=\"f1-brcolor ", "</td>");
					String price = "";
					String tax = "";
					Float oldprice = 0f;
					Float newprice = 0f;
					...
					for (int n = 0; n < pricebufs.length; n++) {
						...
						price = StringUtils.substringBetween(pricebufs[n],
								"data-cell-price-adt=\"", "\"");
						if (StringUtils.isNotEmpty(price)) {
							newprice = Float.parseFloat(price.replaceAll(",",
									""));
							tax = StringUtils.substringBetween(pricebufs[n],
									"data-cell-tax-adt=\"", "\"");
							newtax = Double
									.parseDouble(tax.replaceAll(",", ""));
					...
					System.out.println("Format : " + Dformat.format(d) + " "
							+ Tformat.format(d));
					seg.setFlightno(no);
					...
					fsegs.add(seg);
					// 获取航段
					String fsegBuf[] = getValues(tmp,
							"<tr class=\"flightNextSegment\"", "</tr>");
					for (int n = 0; n < fsegBuf.length; n++) {

						no = StringUtils.substringBetween(fsegBuf[n],
								"data-flight-number=\"", "\"");
						System.out.println("no: " + no);
						// String planeType =
						// StringUtils.substringBetween(tmp,"data-aircraft=\"",
						// "\"");
						dep = StringUtils.substringBetween(fsegBuf[n],
								"data-departureairportcode=\"", "\"");
						arr = StringUtils.substringBetween(fsegBuf[n],
								"data-arrivalairportcode=\"", "\"");
						depTimeStr = StringUtils.substringBetween(fsegBuf[n],
								"data-departuredate=\"", "\"");
						arrTimeStr = StringUtils.substringBetween(fsegBuf[n],
								"data-arrivaldate=\"", "\"");
